...
% $Revision: 1.6 $  $Date: 2015/01/07 21:52:53 $
  ...
  %try
      ...
      % Create RowEditor and get the maximal index of column.
      rd = RowEditor(jtable);
      maxcol = jtable.getColumnCount()-1;
      
      % Add items to ComboBox and assign the ComboBox to the required row.
      SetComboBox (jtable,{'A','B'},0,maxcol,rd);
      SetComboBox (jtable,{'B','C'},1,maxcol,rd);
...
%% SetComboBox
%
% Set items to ComboBox and assign ComboBox to required row. 
%
% inputs:
%       jtable: the instance of JTable
%       data: all items for ComboBox
%       row: the required row index for assigning the ComboBox
%       maxcol: the maximal index of column
%       rd: the instance of RowEditor
%
% example:
%     setComboBox (jtable,{'A','B'},0,maxcol,rd);
%
function SetComboBox (jtable,data,row,maxcol,rd)

      % Create an editable CellEditor.
      cb = javax.swing.JComboBox(data); 
      cb.setEditable(true);      
      editor = javax.swing.DefaultCellEditor(cb); 
      rd.setEditorAt(row,editor);
      
      % Assign the editor to each cell of the required row.
      for i = 0:maxcol     
      jtable.getColumnModel.getColumn(i).setCellEditor(rd);     
      end
%end  % setComboBox
  %% Process optional arguments
function paramsStruct = processArgs(varargin)
    ...
    try
        ...
        paramsStruct.headers = {'Column1','Column2',};  % 3 columns by default
        paramsStruct.data = {'A','B'; 'C','D'};
